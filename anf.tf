...
resource "azurerm_netapp_account" "example_secondary_dual_protocol" {
  count = var.az_dual_protocol_bool ? 1:0
  name                = "${var.az_prefix}-netappaccount-secondary"
  location            = var.az_alt_location
  resource_group_name = azurerm_resource_group.example.name
...
resource "azurerm_netapp_pool" "example_secondary" {
  ...
  location            = var.az_alt_location
  resource_group_name = azurerm_resource_group.example.name
  account_name        = azurerm_netapp_account.example_secondary[0].name
  service_level       = var.az_netapp_pool_service_level_secondary
  size_in_tb          = var.az_capacity_pool_size_secondary
}

resource "azurerm_netapp_pool" "example_secondary_dual_protocol" {
  count = var.az_dual_protocol_bool ? 1:0
  name                = "${var.az_prefix}-netapppool-secondary"
  location            = var.az_alt_location
  resource_group_name = azurerm_resource_group.example.name
  account_name        = azurerm_netapp_account.example_secondary_dual_protocol[0].name
  service_level       = var.az_netapp_pool_service_level_secondary
  size_in_tb          = var.az_capacity_pool_size_secondary
}